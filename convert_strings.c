...
/**
 * convert_s - converts an argument to a string and stores
...
/**
 * convert_S - converts an argument to a string and stores it
 ...
 * Return: the number of bytes stored to the buffer
 *
 * Description: non-printable characters (ASCII values < 32 or
 * >= 127) are stored as \x followed by the ASCII code in hex
 */
unsigned int convert_S(va_list args, buffer_t *output,
				unsigned char flags, int wid, int prec, unsigned char len)
{
